...
import {
  CheckCircleOutlined, CameraOutlined, UserOutlined,
  HistoryOutlined, DownloadOutlined
} from '@ant-design/icons';
import AuditLogDrawer from '../../components/AuditLogDrawer';
import PropTypes from 'prop-types';
import { mockArchivedDispatches } from '../../mocks/_mockData';
import './index.css';
...
const CrmArchive = () => {
  ...
  // Simulate API fetch
  useEffect(() => {
    const fetchArchivedDispatches = async () => {
      try {
        setLoading(true);
        setError(null);
        ...
        // Expected Data: Array of archived dispatch objects
        await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate network delay

        setDispatches(mockArchivedDispatches);
      } catch (err) {
        setError(t('failed_to_load_archived_dispatches'));
        console.error('Failed to load archived dispatch vehicles:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Render payment status (should all be 'paid' for archived items)
  ...
  return (
    <>
      <Card
        title={t('archive_paid')}
        extra={
          <Space>
            <CSVLink
              data={csvData}
              headers={csvHeaders}
              filename={`${t('archive_paid').toLowerCase().replace(' ', '-')}-export.csv`}
            >
              <Button
            ...
            <Text type="secondary">
              {t('showing_completed_paid_dispatches')}
            </Text>
          </Space>
        }
      >
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          <Table
            ...
            locale={{
              emptyText: (
                <Empty
                  description={t('no_archived_dispatches_found')}
